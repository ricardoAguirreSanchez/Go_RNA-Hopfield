...
func main() {
	...
	router.POST("/calcular", func(c *gin.Context) {
		...
		formularioInput := GetInput(c)

		formularioOutput := AplicoBusqueda(formularioInput, algoritmoStruct)

		fmt.Printf("%+v\n", formularioOutput)

		//cargo la pag con resultado
		formularioResultado := FormularioResultado{formularioInput, formularioOutput}
...
func AplicoBusqueda(formularioInput Formulario, algoritmoStruct AlgoritmoStruct) Formulario {

	var formularioRespuesta Formulario
...
*/
func crearPrueba(formu Formulario) mat.Matrix {
	A := mat.NewDense(1, 100, nil)
...
type FormularioResultado struct {
	FormularioInput  Formulario
	FormularioOutput Formulario
}
